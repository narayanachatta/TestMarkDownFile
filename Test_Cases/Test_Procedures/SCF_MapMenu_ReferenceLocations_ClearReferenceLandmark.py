...
if location:
    ...
    # Locate the image on screen
    location = pyautogui.locateOnScreen(os.path.join(test_images_dir, "MapMenu_ReferenceLocations_SetReferenceLandmark.png"), confidence=0.85)
    ...
    if location:
        ...
        # Wait for the Set Reference Landmark Location window
        time.sleep(1)

        # Connect to the window by title
        lat_lan_window = Application(backend="win32").connect(title="Set Reference Landmark Location")

        # Get the window
        lat_lan_window_dlg = lat_lan_window.window(title="Set Reference Landmark Location")
        ...
        # Wait for the Set Reference Landmark Location window to be closed and user to observe the Reference Landmark before it clears
        time.sleep(3)

    else:
        out.write("Menu item 'MapMenu->ReferenceLocations->SetReferenceLandmark' not found.\n")
...
if location:
    ...
    print(f"Clicked at: {center}")
    # Locate the image on screen
    location = pyautogui.locateOnScreen(os.path.join(test_images_dir, "MapMenu_ReferenceLocations_ClearReferenceLandmark.png"), confidence=0.85)
    ...
    if location:
        ...
        # Wait for user to observe the disappear of Reference Landmark
        time.sleep(3)

    else:
        out.write("Menu item 'MapMenu->ReferenceLocations->ClearReferenceLandmark' not found.\n")
        # Close the application and terminate execution
        app.close
        sys.exit()
...
small_img_path  = os.path.join(test_images_dir, "NavMap_ClearReferenceLandmark.png")
