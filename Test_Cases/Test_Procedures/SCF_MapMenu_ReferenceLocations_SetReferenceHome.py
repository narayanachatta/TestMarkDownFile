...
# Right click on Map
mouse.right_click(coords = ((center_x + 10), (center_y + 10)))

# Locate the image on screen
location = pyautogui.locateOnScreen(os.path.join(test_images_dir, "MapMenu_ReferenceLocations.png"), confidence=0.85)
...
if location:
    ...
    # Locate the image on screen
    location = pyautogui.locateOnScreen(os.path.join(test_images_dir, "MapMenu_ReferenceLocations_UseReferenceLocations.png"), confidence=0.85)
    ...
    if location:
        ...
        # Wait for the default Home reference location to display and user to observe
        time.sleep(3)

    else:
        out.write("Menu item 'MapMenu->ReferenceLocations->UseReferenceLocations' not found.\n")
    ...
    if location:
        ...
        location = pyautogui.locateOnScreen(os.path.join(test_images_dir, "MapMenu_ReferenceLocations_SetReferenceHome.png"), confidence=0.85)
        ...
        if location:
            ...
            # Wait for Set Reference Home Location window
            time.sleep(1)
    
            # Connect to the window by title
            lat_lan_window = Application(backend="win32").connect(title="Set Reference Home Location")
    
            # Get the window
            lat_lan_window_dlg = lat_lan_window.window(title="Set Reference Home Location")
    
            # Wait for the window to be in focus
            time.sleep(1)
    
            # Get Latitude textbox
            lat_text = lat_lan_window_dlg.child_window(auto_id="txtLatitude", control_type="System.Windows.Forms.TextBox")
    
            # Enter text
            lat_text.set_edit_text("51.170532")
    
            # Get Longitude textbox
            lon_text = lat_lan_window_dlg.child_window(auto_id="txtLongitude", control_type="System.Windows.Forms.TextBox")
    
            # Enter text
            lon_text.set_edit_text("-1.733646")
    
            # Get OK button
            latlon_ok = lat_lan_window_dlg.child_window(title="Ok", auto_id="buttOK", control_type="System.Windows.Forms.Button")
    
            # Click on OK
            latlon_ok.click_input()
    
            # Wait for the Set Reference Landmark Location window to be closed and user to observe updated Home reference
            time.sleep(3)
    
        else:
            out.write("Menu item 'MapMenu->ReferenceLocations->SetReferenceHome' not found.")
            # Close the application and terminate execution
            app.close
            sys.exit()
        
    else:
        out.write("Menu item 'MapMenu->ReferenceLocations' not found.")
        # Close the application and terminate execution
        app.close
        sys.exit()
...
small_img_path  = os.path.join(test_images_dir, "NavMap_SetReferenceHome.png")
...
if location:
    ...
    if location:
        ...
        # Enter text
        lat_text.set_edit_text("51.161543")

        # Get Longitude textbox
        lon_text = lat_lan_window_dlg.child_window(auto_id="txtLongitude", control_type="System.Windows.Forms.TextBox")

        # Enter text
        lon_text.set_edit_text("-1.719356")
...
# Extract file path of original and small images
original_img_path = os.path.join(original_img_folder, "SCFlight_Window.png")
small_img_path  = os.path.join(test_images_dir, "NavMap_SetReferenceHome_Changed.png")
