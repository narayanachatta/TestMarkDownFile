...
location = pyautogui.locateOnScreen(os.path.join(test_images_dir, "MapMenu_PlanInformation.png"), confidence=0.85)
...
if location:
    ...
    # Wait for Plan Information window and user to observe
    time.sleep(3)

else:
    out.write("Menu item 'MapMenu->PlanInformation' not found.\n")
...
small_img_path  = os.path.join(test_images_dir, "NavMap_No_PlanInformation.png")
...
# Connect to the window by title
plan_info_window = Application(backend="win32").connect(title="Plan Information")

# Get the window
plan_info_dlg = plan_info_window.window(title="Plan Information")

# Get OK button dialog
plan_info_ok = plan_info_dlg.child_window(title="OK", class_name="Button")

# Click on OK button
plan_info_ok.click_input()

# Wait for Plan Information window to close
time.sleep(1)
...
# Wait for the mission plan to be loaded
time.sleep(2)
...
# Extract file path of original and small images
original_img_path = os.path.join(original_img_folder, "SCFlight_Window.png")
small_img_path  = os.path.join(test_images_dir, "NavMap_With_PlanInformation.png")
