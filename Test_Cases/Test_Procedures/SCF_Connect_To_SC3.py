...
# Get Flight Phase label
phase = main_window.child_window(auto_id="lightPhase", control_type="Gcs.FormControls.IndicatorLight")
phase_label = phase.child_window(auto_id="text_label", control_type="System.Windows.Forms.Label")

# Check the Flight Phase changed to DISARMED
if "DISARMED" == phase_label.window_text():
    out.write('----------------------------\n')
    out.write(f'Flight Phase changed to {phase_label.window_text()}\n')
    out.write('----------------------------\n')

# Get Flight Mode label
mode = main_window.child_window(auto_id="lightMode", control_type="Gcs.FormControls.IndicatorLight")
mode_label = mode.child_window(auto_id="text_label", control_type="System.Windows.Forms.Label")

# Check the Flight Mode changed to DISARMED
if "ACRO" == mode_label.window_text():
    out.write('----------------------------\n')
    out.write(f'Flight Mode changed to {mode_label.window_text()}\n')
    out.write('----------------------------\n')
...
small_img_path  = os.path.join(test_images_dir, "Phase_And_Mode_When_Connected_To_SC3.png")
